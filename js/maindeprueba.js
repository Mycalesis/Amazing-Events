//main de testeo
/// acordate de ordenar las carpetas
...
for(let event of data.events){
    ...
    if (eventDate < currentDate) {
        ...
        // Si no hay categorías seleccionadas, se muestran todos los eventos
            ...
            let listOfEvents = '';
            let listOfEventPast = '';
            let listOfEventFuture = '';
            data.events.forEach(event => {
                for(let event of data.events){
                    let currentDate = new Date(data.currentDate);
                    let eventDate = new Date(event.date);
                    if (eventDate < currentDate) {
                        listOfEventPast += `<div class="card m-2 py-2" style="width: 18rem">
                                        <img src = ${event.image} class="card-img-top imageCard" ">
                                        <div class="card-body">
                                            <h5 class="card-title">${event.name}</h5>
                                            <p class="card-text">${event.category}</p>
                                ...
                                listOfEventFuture += `<div class="card m-2 py-2" style="width: 18rem">
                                <img src = ${event.image} class="card-img-top imageCard" ">
                                <div class="card-body">
                                    <h5 class="card-title">${event.name}</h5>
                                    <p class="card-text">${event.category}</p>
        ...
        // Actualiza el contenido del elemento que contiene la lista de eventos para cada
                let containerOfCard = document.querySelector('div.eventcard.row'); 
